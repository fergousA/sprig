...
#let showcase(file, pdf, title, why) = {
  ...
  v(0.5em)
  block(fill: rgb("#F5F3EE"), inset: 7pt, radius: 3pt, width: 100%,
    // Shown VERBATIM. The examples used to import `../lib.typ` — correct
    // inside the package, wrong for a reader copying the block, who has no
    // such sibling — so this line rewrote it on the way to the page. The
    // files now carry the published import themselves, as Universe
    // requires, and the substitution would be a no-op: better to read the
    // file as it is than to keep a rewrite that silently matches nothing.
